...
public class WebSecurityConfig {
	...
	/*
	 * final 키워드는 불편객체로 설정 JwtAuthenticationFilter는 우리가 만든 jwt 기반 인증 필터이고, 이 필터를 스프링
	 * 시큐리티 필터체인에 추가하기 위해 생성자 주입 방식으로 WebSecurityConfig에 전달하는 방식
	 */
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http, CustomOAuth2UserService customOAuth2UserService)
		...
		http.cors(cors -> {
				...
				// OAuth2 로그인 설정
				.oauth2Login(oauth2 -> oauth2.userInfoEndpoint(userInfo -> userInfo.userService(customOAuth2UserService) // OAuth2
																															// 사용자
																															// 정보
																															// 로딩
