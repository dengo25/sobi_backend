...
public class MyPageController {
	...
	public ResponseEntity<?> getMypage(@AuthenticationPrincipal String userId) {
		try {
			...
			if (member != null) {
				MemberDTO responseMemberDTO = MemberDTO.builder().id(member.getId()).memberId(member.getMemberId())
						.memberName(member.getMemberName()).memberEmail(member.getMemberEmail())
	...
	public ResponseEntity<?> updateMypage(@AuthenticationPrincipal String userId, @RequestBody MemberDTO memberDTO) {
		try {
			...
			MemberDTO responseMemberDTO = MemberDTO.builder().id(updatedMember.getId())
					.memberId(updatedMember.getMemberId()).memberName(updatedMember.getMemberName())
					.memberEmail(updatedMember.getMemberEmail()).memberGender(updatedMember.getMemberGender()) // 성별 추가
					...
					.role(updatedMember.getRole()).build();
