...
//    }
		...
		if (!memberRepository.existsByMemberIdAndIsActive(username, "Y")) {
			member = Member.builder().memberId(username).memberEmail(email).memberName(name) // 소셜 로그인 시 이름도 저장
					.isActive("Y") // 명시적으로 활성 상태 설정
					.build();
			member = memberRepository.save(member); // Db에 저장
		} else {
			member = memberRepository.findByMemberIdAndIsActive(username, "Y"); // 기존 활성 사용자 조회
		}
		...
		return new CustomMember(member.getId(), member.getMemberId(), member.getMemberName(), member.getMemberEmail(),
				member.getRole(), authorities, attributes);
